...
{
    int entry_index = physical_ram[get_page_start_addr(ppage) + entry];
...
{
    int paddr = get_page_table_entry(dir_page_num, ((vaddr >> 16) & 0xFF));

    int dir_num = get_page_table_entry(paddr, ((vaddr >> 8) & 0xFF));

    return (get_page_start_addr(dir_num) + (vaddr & 0xFF));
}
/*
 * Main
 */
