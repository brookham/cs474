...
struct inode *ialloc(void){
    ...
    struct inode *in = iget(&(struct inode){.inode_num = free_bit});
    if (in == NULL){
        return NULL;
    }
...
struct inode *iget(struct inode *in){
    struct inode *found = incore_find(in->inode_num);
    if (found != NULL){
        found->ref_count++;
        return found;
    }
    ...
    read_inode(free_in, in->inode_num);
    free_in->ref_count = 1;
    free_in->inode_num = in->inode_num;
    return free_in;
}
